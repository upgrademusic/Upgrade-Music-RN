...
export default function EventDetailScreen() {
  ...
  }>({ open: false, song: null, mode: 'request', existingGroupId: null, originatorId: null });
  const [selectedAmount, setSelectedAmount] = useState(500);
  const [customAmount, setCustomAmount] = useState('');
  const [submitting, setSubmitting] = useState(false);

  const openBid = useCallback((song: SearchSong) => {
    const isBoost = song.inQueue;
    setBidModal({
      open: true,
      song,
      mode: isBoost ? 'boost' : 'request',
      existingGroupId: song.requestGroupId,
      originatorId: song.originatorId,
    });
  ...
  const submitRequest = useCallback(async () => {
    if (!user || !eventId) { Alert.alert('Error', 'You must be logged in'); return; }
    if (!bidModal.song) return;

    const amountCents = customAmount
      ? Math.max(100, Math.round(parseFloat(customAmount) * 100))
      : selectedAmount;
    ...
    try {
      ...
      if (song.spotifyId) {
        ...
        if (existing) {
          ...
          const { data: newSong, error } = await supabase
            ...
            .select('id')
            .single();
          if (error || !newSong) throw new Error('Failed to add song');
          songId = newSong.id;
        }
      }
      ...
      const { data: groupId, error: rpcErr } = await supabase.rpc('increment_request_group', {
        ...
        p_is_boost: bidModal.mode === 'boost',
      });
      ...
      if (bidModal.mode === 'boost' && bidModal.originatorId) {
        ...
        if (reward > 0) {
          await supabase.from('originator_rewards').insert({
            event_id: eventId,
            song_id: songId,
            originator_id: bidModal.originatorId,
            booster_id: user.id,
            boost_amount_cents: amountCents,
            reward_cents: reward,
            status: 'pending',
          });
        }
      ...
      setBidModal(m => ({ ...m, open: false }));
      setQuery('');
      Alert.alert(
        '🎵 Requested!',
        bidModal.mode === 'boost'
          ? `Boosted "${song.title}" with ${formatCents(amountCents)}!`
          : `Requested "${song.title}" for ${formatCents(amountCents)}!`
      );
    } catch (e: any) {
  ...
  }, [bidModal, selectedAmount, customAmount, user, eventId, eventInfo, initPaymentSheet, presentPaymentSheet]);
      ...
      <SafeAreaView style={styles.safe}>
        <View style={styles.header}>
          ...
          <View style={styles.headerInfo}>
            ...
            {eventInfo?.venueName && (
              <Text style={styles.venueName}>{eventInfo.venueName}</Text>
            )}
          </View>
            ...
            <View style={styles.modalSheet}>
              ...
              {bidModal.song && (
                <View style={styles.modalSongRow}>
                  <Image
                    source={{ uri: bidModal.song.albumArt ?? '' }}
              ...
              {bidModal.mode === 'boost' && bidModal.originatorId && (
                <Text style={styles.originatorNote}>
                  ✨ 5% goes to the original requester as a reward
                </Text>
              )}
              ...
              <View style={styles.modalActions}>
                <TouchableOpacity
                  ...
                  onPress={() => setBidModal(m => ({ ...m, open: false }))}
                >
...
const styles = StyleSheet.create({
  ...
  originatorNote: { color: Colors.purple.light, fontSize: 13, textAlign: 'center', marginBottom: Spacing.md },
